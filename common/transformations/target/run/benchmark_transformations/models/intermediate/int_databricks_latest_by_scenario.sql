  ...
  create view "benchmark_results"."main"."int_databricks_latest_by_scenario__dbt_tmp" as (
    

-- Get latest Databricks run for each scenario (first run_num only)
WITH latest_runs AS (
    SELECT
        scenario,
        MAX(timestamp) AS latest_timestamp
    FROM "benchmark_results"."main"."base_databricks_results"
    GROUP BY scenario
),
...
latest_run_ids AS (
    SELECT DISTINCT
        r.scenario,
        r.run_id
    FROM "benchmark_results"."main"."base_databricks_results" r
