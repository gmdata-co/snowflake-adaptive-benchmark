...
QUERY_PARAMS = {
    ...
    14: {':1': '1995-09-01'},
    15: {':1': '1996-01-01'},
    16: {':1': 'Brand#45', ':2': 'MEDIUM POLISHED', ':3': '49', ':4': '14', ':5': '23', ':6': '45', ':7': '19', ':8': '3', ':9': '36', ':10': '9'},
...
def clean_query(query_text: str, query_num: int) -> str:
    ...
    if query_num in QUERY_PARAMS:
        for placeholder, value in QUERY_PARAMS[query_num].items():
            ...
            query = query.replace(f"':1' day (3)", f"'{value}' day")  # Special case for Q1
            query = query.replace(placeholder, value)

    # Fix interval syntax for Snowflake (remove precision specifier)
    query = re.sub(r"interval\s+'(\d+)'\s+day\s+\(\d+\)", r"interval '\1 days'", query, flags=re.IGNORECASE)
